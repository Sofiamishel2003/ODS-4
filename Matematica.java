...
public class Matematica extends Estudiante{
    ...
    public void preguntas(int grado){
        ...
        while(j != 5){
            ...
            int random = rand.nextInt(1,10) + 1;
            String respuesta = "";
            ...
            if(bandera == false){
                randoms[j] = random;
                j = j + 1;
                System.out.println(j + ". " + preguntasMate[grado - 1][random]);
                System.out.println("-a. " + respuestasMate[random + ((grado - 1) * (10))][0]);
                System.out.println("-b. " + respuestasMate[random + ((grado - 1) * (10))][1]);
                System.out.println("-c. " + respuestasMate[random + ((grado - 1) * (10))][2]);
                respuesta = keyboard.nextLine();
                if(respuesta == "a"){
                    respuesta = respuestasMate[random + ((grado - 1) * (10))][0];
                }else if(respuesta == "b"){
                    respuesta = respuestasMate[random + ((grado - 1) * (10))][1]; 
                }else if(respuesta == "c"){
                respuesta = respuestasMate[random + ((grado - 1) * (10))][2];
                }else{
                    System.out.println("Respuesta inválida");
                }
                for(int i = 0; i<10 ; i++){
                    if(respuesta == respuestasCorrectasMate[grado - 1][i]){
                        bandera2 = true;
                    }
                }
                if(bandera2 == true){
